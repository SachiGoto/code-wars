...
function allNonConsecutive (arr) {
    ...
    // elemets are numbers, unique and in ascending order
    // # could be positive or negative, gap can be larger than one
    // if there is only one element or empty => return empty array
    //   eg. [1,2,3,6,10, 11]
    //   return [
    //      {i:2, n:6},
    //      {i:4, n:10}
    //   ]
      ...
      for(let i=0; i<arr.length-1; i++){
        if(arr[i] + 1  !== arr[i+1] ) newArr.push({i:i+1, n:arr[i+1]})
